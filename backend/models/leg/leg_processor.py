...
def process_leg_frame(frame):
    ...
    for side, ids in legs.items():
        if all(keypoints[i, 2] > MIN_CONF for i in ids):
            ...
            if smooth_angle > EXT_THRESH and states[side] == "down":
                ...
                feedback[side] = f"Rep {counters[side]}: {res}"
                persist_frames[side] = 60

            # --- DATA COLLECTION FIX: EXACTLY 6 POINTS ---
            if collecting_sides[side] and buffers[side]:
                ...
                # Unpacking exactly 6 points satisfies the 'takes 7 positional arguments' (6 + self)
    ...
    for s in ["left", "right"]:
        if persist_frames[s] > 0: persist_frames[s] -= 1
